...
def main(infer, infer1, np_img, TYPE_LIST, AREA_LIST, is_pose):
    try:
        ...
        if TYPE_LIST[2] and is_pose:
            if points[:, 11 * 3 + 1].all() != 0 and points[:, 12 * 3 + 1].all() != 0 and points[:, 15 * 3 + 1].all() != 0 and points[:, 16 * 3 + 1].all() != 0:
                # ---------------------------------------------------------#
                #   跌倒决策1
                # ---------------------------------------------------------#
        ...
        if TYPE_LIST[3] and is_pose:
            ...
            if right_hand.shape[0] == right_hand_pre.shape[0] and left_hand.shape[0] != 0 and left_hand_pre.shape[0] != 0:
            ...
            if left_angle_vari.shape[0] == T.shape[0]:
                ...
                cond2 = left_pos_y < 0.08 * h_
                cond3 = left_angle > 80
                cond4 = np.abs(right_angle_vari) > 20
                cond5 = right_pos_y < 0.08 * h_
                cond6 = right_angle > 80
                # 低于头部
                cond7 = left_elbow[:, 1] > head_y
                cond8 = right_elbow[:, 1] > head_y
                # y轴速度限制
                cond9 = np.abs(v_left_hand) < 4 * h_
                cond10 = np.abs(v_right_hand) < 4 * h_
                # x轴速度
                cond_11 = np.abs(v_left_hand_x) > 1 * w_
                cond_12 = np.abs(v_right_hand_x) > 1 * w_
...
if __name__ == '__main__':
    ...
    framerate = 60
    flip_method = 0
    TYPE_LIST = [True, True, True, True, True, False]
    ...
    infer = LoadPoseEngine('yolov8s-pose.engine')
    infer1 = LoadEngineModel('detect3.engine')
    print('模型加载完成！')

    # ---------------------------------------------------------#
    #   循环
    # ---------------------------------------------------------#
    # cap = cv2.VideoCapture('image/smoke2.mp4')
    ...
    while 1:
        ...
        if np_img is not None:
            i += 1
            (np_img, RES_LIST) = main(infer, infer1, np_img, TYPE_LIST, AREA_LIST, is_pose)
            print(TYPE_LIST, RES_LIST)
        end_i = time.time()
