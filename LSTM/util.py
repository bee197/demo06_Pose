...
def read_xy(file, max_body=2, num_joint=25):
    ...
    data = np.zeros((2, seq_info['numFrame'], num_joint, max_body))  # 初始化数据； 3 × 帧数 × 25 × max_body
    for n, f in enumerate(seq_info['frameInfo']):  # 遍历每一帧的数据
        for m, b in enumerate(f['bodyInfo']):  # 遍历每一个body的数据
            for j, v in enumerate(b['jointInfo']):  # 遍历每一个关节的数据
                if m < max_body and j < num_joint:
                    if math.isnan(v['colorX']) or math.isnan(v['colorY']):
                        data[:, n, j, m] = [0, 0]  # 保存 x,y2个坐标的数据
                    else:
                        data[:, n, j, m] = [v['colorX']/1920., v['colorY']/1080.]  # 保存 x,y2个坐标的数据
                else:
                    pass
    return data
...
def Print2D(num_frame, point, arms, rightHand, leftHand, legs, body):
    ...
    for i in range(n, m):
        ...
        # 画出两个body所有关节
        plt.scatter(point[0, i, arms, :], point[1, i, arms, :], c='red', s=40.0)  # c: 颜色;  s: 大小
        plt.scatter(point[0, i, legs, :], point[1, i, legs, :], c='red', s=40.0)  # c: 颜色;  s: 大小
        plt.scatter(point[0, i, body, :], point[1, i, body, :], c='red', s=40.0)  # c: 颜色;  s: 大小
...
def main():
    ...
    # sys.path.extend(['data/'])  # 扩展路径
    data_path = 'data/S007C001P025R002A050.skeleton'  # 拍手skeleton文件名
    # data_path = 'S001C001P001R001A058.skeleton' # 握手skeleton文件名，two body
    point = read_xy(data_path)  # 读取 x,y,z三个坐标
    ...
    # 相邻关节标号
    arms = [11, 9, 8, 4, 5, 7]  # 23 <-> 11 <-> 10 ...
    rightHand = []  # 11 <-> 24
    leftHand = []  # 7 <-> 22
    legs = [18, 17, 16, 12, 13, 14]  # 19 <-> 18 <-> 17 ...
    body = [3, 8, 16, 12, 4, 3]  # 3 <-> 2 <-> 20 ...
